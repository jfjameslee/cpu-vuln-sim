...
use super::sim::{
    BranchPredictorState, SpectreSim, SpectrePhase, CACHE_HIT_CYCLES, CACHE_MISS_CYCLES,
    ARRAY1_BASE, SECRET_ADDR, PROBE_ARRAY_BASE, TRAIN_ROUNDS,
};

pub(super) fn draw(frame: &mut Frame, state: &SpectreSim) {
...
fn render_header(frame: &mut Frame, state: &SpectreSim, area: ratatui::layout::Rect) {
    ...
    let lines = vec![
        Line::from(vec![
            Span::styled(
                "  SPECTRE V1 VULNERABILITY SIMULATOR",
                Style::default().fg(Color::White).add_modifier(Modifier::BOLD),
            ),
            Span::styled("  (CVE-2017-5753)", Style::default().fg(Color::DarkGray)),
        ]),
        Line::from(vec![
...
fn render_assembly(frame: &mut Frame, state: &SpectreSim, area: ratatui::layout::Rect) {
    ...
    match &state.phase {
        SpectrePhase::Train { round } => {
            let (bp_label, bp_color) = match state.branch_predictor {
                BranchPredictorState::Trained => (
                    format!(" Branch predictor: TRAINED ({}/{} rounds)", round, TRAIN_ROUNDS),
                    Color::Green,
                ),
                _ => (
                    format!(
                        " Branch predictor: TRAINING ({}/{} rounds complete)",
                        round, TRAIN_ROUNDS
                    ),
                    Color::Yellow,
                ),
            ...
            lines.push(Line::from(vec![Span::styled(
                bp_label,
                Style::default().fg(bp_color).add_modifier(Modifier::BOLD),
            )]));
        }
        SpectrePhase::Speculative { step } if *step > 0 && *step < 5 => {
            lines.push(Line::from(vec![Span::styled(
                " \u{26a1} Speculative window active \u{2014} CPU executing past mispredicted branch",
                Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD),
            )]));
        }
        SpectrePhase::Reload { .. } | SpectrePhase::Revealed => {
            lines.push(Line::from(vec![Span::styled(
                " \u{2717} Pipeline flushed \u{2014} registers rolled back, cache persists",
...
fn render_cache(frame: &mut Frame, state: &SpectreSim, area: ratatui::layout::Rect) {
    let mut lines: Vec<Line> = Vec::new();
...
fn render_status(frame: &mut Frame, state: &SpectreSim, area: ratatui::layout::Rect) {
    let inner = Layout::vertical([Constraint::Length(15), Constraint::Fill(1)]);
    let block = Block::new()
        .borders(Borders::ALL)
        .border_style(Style::default().fg(Color::DarkGray))
        .title(Span::styled(
    ...
    let (bp_label, bp_color) = match state.branch_predictor {
        BranchPredictorState::Untrained => ("UNTRAINED", Color::DarkGray),
        BranchPredictorState::Training => ("TRAINING", Color::Yellow),
        BranchPredictorState::Trained => ("TRAINED", Color::Green),
        BranchPredictorState::Mispredicting => ("MISPREDICTED", Color::Red),
    };

    let bp_rounds = match &state.phase {
        SpectrePhase::Train { round } => format!(" ({}/{})", round, TRAIN_ROUNDS),
        SpectrePhase::Speculative { .. } | SpectrePhase::Reload { .. } | SpectrePhase::Revealed => {
            format!(" ({}/{})", TRAIN_ROUNDS, TRAIN_ROUNDS)
        }
        _ => String::new(),
    };
    ...
    let mut map_lines: Vec<Line> = vec![
        ...
        Line::from(vec![
            Span::styled("   array1          ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("{ARRAY1_BASE:#018x}"), Style::default().fg(Color::Cyan)),
            Span::styled("  (16 bytes, bounds-checked)", Style::default().fg(Color::DarkGray)),
        ]),
        Line::from(
            vec![
                Span::styled("   [secret]        ", Style::default().fg(Color::DarkGray)),
                Span::styled(format!("{SECRET_ADDR:#018x}"), Style::default().fg(Color::Yellow)),
                Span::raw("  =  "),
            ]
            .into_iter()
        ...
        Line::from(vec![
            Span::styled("   array2 (probe)  ", Style::default().fg(Color::DarkGray)),
            Span::styled(format!("{PROBE_ARRAY_BASE:#018x}"), Style::default().fg(Color::Cyan)),
            Span::styled("  (256 \u{d7} 512 B)", Style::default().fg(Color::DarkGray)),
        ]),
        Line::from(""),
        Line::from(vec![
            Span::styled(" Branch Predictor: ", Style::default().fg(Color::White)),
            Span::styled(bp_label, Style::default().fg(bp_color).add_modifier(Modifier::BOLD)),
            Span::styled(bp_rounds, Style::default().fg(Color::DarkGray)),
        ]),
        Line::from(""),
        ...
        register_spans("RDI", &state.registers.rdi),
    ...
    if let SpectrePhase::Reload { step } = &state.phase {
